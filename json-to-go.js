...
function jsonToGov2(json, options = {}) {
	...
	const smallInts = options.smallInts === undefined ? false : options.allOmitempty
	...
	function goType(val)
	{
		...
		switch (typeof val)
		{
			...
			case "number":
				...
				if (smallInts && val % 1 === 0 && val >= -128 && val <= 127)
					return "int8"
				if (smallInts && val % 1 === 0 && val >= -32768  && val <= 32767)
					return "int16"
				if (val % 1 === 0 && val >= -2147483648 && val <= 2147483647)
					return smallInts ? "int32" : "int" // use "int" for backwards compatibility
...
Optional arguments:
	...
	--small-ints         Instead of only using int32 and int64, also use int8 and int16. This might save some memory,
                         but can easily cause number overflows if the example json does not have proper example values
		...
		process.argv.forEach((val, index) => {
			...
			switch (argument.arg) {
				...
				case "all-omitempty":
					 ...
					 break
				case "small-ints":
					options.smallInts = argument.value
					break
				case "help":
					printHelp()
					process.exit(0)
